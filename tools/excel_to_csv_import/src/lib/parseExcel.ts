...
import {
  ...
  type CardColumnMatch,
  type NormalizedTransaction,
} from "./mapping.js";
import { createReportCollector, type ImportReport } from "./report.js";
...
export function convertExcelToCsv(options: ConvertOptions): ConvertResult {
  const report = createReportCollector();
  const cardsConfig = JSON.parse(fs.readFileSync(options.cardsConfigPath, "utf8")) as unknown;
  ...
  const cardColumns: Array<CardColumnMatch & { columnIndex: number }> = [];
  ...
  matrix.slice(1).forEach((row, rowOffset) => {
    ...
    const rowTransactions: Array<Omit<NormalizedTransaction, "importKey">> = [];
    let hasIncome = false;
    ...
    const forceTransactionsCategory =
      mappedCategory.originalKey === "none" || (hasIncome && hasOutcome);
    ...
    rowTransactions.forEach((transaction) => {
      ...
      const finalizedTransaction: NormalizedTransaction = {
        ...transaction,
        category,
