...
export interface CardColumnMatch {
  header: string;
  cardName: string;
  cardId: number;
...
const outcomeCategorySet = new Set<string>(OUTCOME_CATEGORIES);
const incomeCategorySet = new Set<string>(INCOME_CATEGORIES);
...
export function parseCardsConfig(rawConfig: unknown): CardDefinition[] {
...
export function mapOldCategory(rawCategory: unknown, report: ReportCollector): MappedCategoryResult {
...
export function validateCategoryForType(category: string, type: TransactionType): string {
...
export function detectCardColumn(
  header: string,
  cards: CardDefinition[],
  report: ReportCollector,
): CardColumnMatch | null {
  ...
  const type: TransactionType = hasIncome ? "income" : "outcome";
...
export function buildImportKey(transaction: Omit<NormalizedTransaction, "importKey">, sourceRowIndex: number): string {
  return [
    transaction.date,
    transaction.card,
    transaction.type,
...
function buildCardAliases(displayName: string): string[] {
...
function isDirectionalPersonLabel(value: string): boolean {
